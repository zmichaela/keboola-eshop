# Deploying Web Apps to Keboola Data Apps (Full Reference)
...
**Nginx:** Must include WebSocket upgrade headers. Streamlit uses WebSockets for `/_stcore/stream`.
...
**Fix:** Ensure `setup.sh` is executable (`chmod +x`), paths in Supervisord are absolute (`/app/...`), `uv run` prefixes all Python commands, `uv sync` succeeds, and Nginx proxies to the correct internal port.
